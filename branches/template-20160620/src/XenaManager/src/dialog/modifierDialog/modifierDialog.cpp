...
CModifierDialog::CModifierDialog(const ModifierData & data, QWidget *parent /*= 0*/, Qt::WindowFlags f /*= 0*/)
: QDialog(parent, f)
{
    m_data = data;
...
void CModifierDialog::OnPosEditFinished()
{
    ...
    if (val > maxVal) {
        m_editPos->setText(QString::number(m_segInfo.pos-m_data.pos));
        m_editPos->setStyleSheet("color:black");
    }
    else {
