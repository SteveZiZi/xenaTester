// sclManager.cpp : 定义 DLL 应用程序的导出函数。
//
#include "include/sclManager.h"
...
///     0 - 操作成功； -1 - 操作失败
///
int CSCLManager::Load(const std::wstring &strFileName, bool bForce/* = false*/)
{
    if (!bForce && m_strFileName == strFileName) {
        return 0;
    }
    ...
    if ( 0 == m_pSclCache->LoadSCL(strFileName) ) {
