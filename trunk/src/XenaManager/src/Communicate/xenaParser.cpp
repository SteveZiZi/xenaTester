...
bool CXenaParser::dealModuleAndPortReply(const QStringList & infoList)
{
    ...
    else if (moduleAndPortInfo.contains("PS_",Qt::CaseInsensitive))
    {
        return dealStreamAbnormalReply(infoList);
    }
...
bool CXenaParser::dealPortReply(const QStringList & portInfoList)
{
    ...
    else if (commandStr.contains("P_TXMODE",Qt::CaseInsensitive)){
        m_result = RL_P_TXMODE;
    }
    else if (commandStr.contains("P_RATEFRACTION",Qt::CaseInsensitive)){
        m_result = RL_P_RATEFRACTION;
    }
    else if (commandStr.contains("P_RATEPPS",Qt::CaseInsensitive)){
        m_result = RL_P_RATEPPS;
    }
    else if (commandStr.contains("P_RATEL2BPS",Qt::CaseInsensitive)){
        m_result = RL_P_RATEL2BPS;
    }
    else {
        return false;
    }
...
bool CXenaParser::dealStreamReply(const QStringList & streamInfoList)
{
    ...
    else if (commandStr.contains("PS_PACKETHEADER",Qt::CaseInsensitive)){
        m_result = RL_PS_PACKETHEADER;
    }
    else if (commandStr.contains("PS_TPLDID",Qt::CaseInsensitive)){
        m_result = RL_PS_TPLDID;
    }
    // 获取流的规约
    else if (commandStr.contains("PS_HEADERPROTOCOL",Qt::CaseInsensitive))
    {
        m_result = RL_PS_HEADERPROTOCOL;
    }
    // 获取流长度
    else if (commandStr.contains("PS_PACKETLENGTH",Qt::CaseInsensitive))
    {
        m_result = RL_PS_PACKETLENGTH;
    }
    // 获取流跳变域
    else if ("PS_MODIFIER" == commandStr || "ps_modifier" == commandStr)
    {
        QStringList modifireAndStreamList = streamInfoList[2].split(",");
        int streamIndex = getNumberFormString(modifireAndStreamList[0]);
        int modifierIndex = getNumberFormString(modifireAndStreamList[1]);
        m_sIndex = streamIndex;
        m_miIndex = modifierIndex;
        m_result = RL_PS_MODIFIER;
    }
    // 获取流跳变域的变化范围
    else if (commandStr.contains("PS_MODIFIERRANGE",Qt::CaseInsensitive))
    {
        QStringList modifireAndStreamList = streamInfoList[2].split(",");
        int streamIndex = getNumberFormString(modifireAndStreamList[0]);
        int modifierIndex = getNumberFormString(modifireAndStreamList[1]);
...
bool CXenaParser::dealStreamAbnormalReply(const QStringList & streamCountList)
{
    const QString &commandStr = streamCountList[1];
    const QString &moduleIndexAndPortIndex = streamCountList[0];
    ...
    QStringList moduleAndPortList = streamCountList[0].split("/");
    int moduleIndex = moduleAndPortList[0].toInt();
    int portIndex = moduleAndPortList[1].toInt();
    const QString &signalStr = streamCountList[2];

    m_mIndex = moduleIndex;
    m_pIndex = portIndex;
    m_text = signalStr;
    removeInvisibleSymbols(m_text);
    if (commandStr.contains("PS_INDICES",Qt::CaseInsensitive)) {
        m_result = RL_PS_STREAM_NUM;
    }else{
        return false;
    }
...
void CXenaParser::removeInvisibleSymbols(QString &text)
{
    text.remove('\"');
    text.remove('\n');
}