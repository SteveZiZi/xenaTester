...
QByteArray ScriptCommand::cmdToByteArray(E_ORDER type,S_ORDER & order)
{
	...
	case C_RESERVEDBY:
		...
		break;
    case C_INFO:
        command = getChassisInfo();
        break;
    case C_CONFIG:
        command = getChassisConifg();
        break;
    case C_STATS:
        command = getChassisStats();
        break;
	...
	case M_MODEL:
		...
		break;
    case M_INFO:
        command = getModuleInfo(order);
        break;
    case M_CONFIG:
        command = getModuleConfig(order);
        break;
    case M_STATUS:
        command = getModuleStatus(order);
        break;
	...
	case P_SPEED:
		...
		break;
    case P_INFO:
        command = getPortInfo(order);
        break;
    case P_CONFIG:
        command = getPortConfig(order);
        break;
    case P_FULLCONIFG:
        command = getPortFullConfig(order);
        break;
    case P_TXMODE_QUERY:
        command = getTxMode(order);
        break;
    case P_TXMODE_NORMAL:
        command = setTxModeNormal(order);
        break;
    case P_TXMODE_STRICUNIFORM:
        command = setTxModeStrict(order);
        break;
    case P_TXMODE_SEQUENTIAL:
        command = setTxModeSequential(order);
        break;
    case P_RATEFRACTION_QUERY:
        command = getPortRateFraction(order);
        break;
    case P_RATEFRACTION_SET:
        command = setPortRateFraction(order);
        break;
    case P_RATEPPS_QUERY:
        command = getPortPacketRate(order);
        break;
    case P_RATEPPS_SET:
        command = setPortPacketRate(order);
        break;
    case P_RATEL2BPS_QUERY:
        command = getPortBitRate(order);
        break;
    case P_RATEL2BPS_SET:
        command = setPortBitRate(order);
        break;
	case PS_CREATE:
	...
	case PS_TPLDID:
		...
		break;
    case PS_COMMENT:
        command = setStreamComment(order);
        break;
...
QString ScriptCommand::getChassisInfo()
{
    QString command("C_INFO ?");
    return command;
}

QString ScriptCommand::getChassisConifg()
{
    QString command("C_CONFIG ?");
    return command;
}

QString ScriptCommand::getChassisStats()
{
    QString command("C_STATS ?");
    return command;
}
...
QString ScriptCommand::getModuleInfo(const S_ORDER & order)
{
    QString command = composeModuleCommandHead(order);
    command += " M_INFO ?";
    return command;
...
QString ScriptCommand::getModuleConfig(const S_ORDER & order)
{
    QString command = composeModuleCommandHead(order);
    command += " M_CONFIG ?";
    return command;
...
QString ScriptCommand::getModuleStatus(const S_ORDER & order)
{
    QString command = composeModuleCommandHead(order);
    command += " M_STATUS ?";
    return command;
...
QString ScriptCommand::getPortInfo(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_INFO ?";
    return command;
...
QString ScriptCommand::getPortConfig(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_CONFIG ?";
    return command;
...
QString ScriptCommand::getPortFullConfig(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_FULLCONFIG ?";
    return command;
}

QString ScriptCommand::getTxMode(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_TXMODE ?";
    return command;
...
QString ScriptCommand::setTxModeNormal(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_TXMODE NORMAL";
    return command;
}

QString ScriptCommand::setTxModeStrict(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_TXMODE STRICTUNIFORM";
    return command;
}

QString ScriptCommand::setTxModeSequential(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_TXMODE SEQUENTIAL";
    return command;
}

QString ScriptCommand::getPortRateFraction(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_RATEFRACTION ?";
    return command;
}

QString ScriptCommand::setPortRateFraction(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_RATEFRACTION ";
    command += order.data;
...
QString ScriptCommand::getPortPacketRate(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_RATEPPS ?";
    return command;
...
QString ScriptCommand::setPortPacketRate(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_RATEPPS ";
    command += order.data;
...
QString ScriptCommand::getPortBitRate(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_RATEL2BPS ?";
    return command;
...
QString ScriptCommand::setPortBitRate(const S_ORDER & order)
{
    QString command = composePortCommandHead(order);
    command += " P_RATEL2BPS ";
    command += order.data;
...
QString ScriptCommand::setStreamComment(const S_ORDER & order)
{
    QString command = composeStraeamCommandHead(order);
    command += " PS_COMMENT [";
    command += QString::number(order.streamindex);
...
QString ScriptCommand::getStreamConfig(const S_ORDER & order)
{
    QString command = composeStraeamCommandHead(order);
    command += " PS_CONFIG [";
    command += QString::number(order.streamindex);
