    ...
    if (info.pos+info.length <= pos) {
        ...
        if (siblingItem->rowCount()) {
            for(int row = 0; row < siblingItem->rowCount(); row++) {
                ...
                if (childItem) {
                    if (!childItem->text().contains("Modifier",Qt::CaseInsensitive)){
                        return childItem;
                    }
                    else {
                        return siblingItem;
...
bool ethFrameToImportData(const unsigned char* frame, unsigned int size, ImportData* importData)
{
    ...
    if(frame == NULL || importData == NULL) {
        return false;
    }

    switch (msgDecoder.GetMessageType(frame, size)) {
    case FRAME_TYPE_NET_9_2:
        importData->frameStyle = "SMV92";
        break;
    case FRAME_TYPE_NET_GOOSE:
        importData->frameStyle = "GOOSE";
        break;
    default:
        importData->frameStyle = "UNKNOWN";
        break;
    }
    msgDecoder.GetAppID(frame, size, importData->appId);
    importData->iedName = "UNKNOWN";
    importData->frameLength = size;
    delete[] importData->frame;
    importData->frame = new quint8[size];
    memcpy(importData->frame, frame, size);
